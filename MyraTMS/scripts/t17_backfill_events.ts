...
const PIPELINE_LOADS_STEPS: BackfillStep[] = [
  {
    ...
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'load.scanned', 'load', id, id, 'system', 'system',
  ...
  },
  {
    table: 'pipeline_loads',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'load.stage_changed', 'load', id, id, 'system', 'system',
            jsonb_build_object('load_id', load_id, 'source', load_board_source),
  ...
  },
  {
    table: 'pipeline_loads',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, fn_stage_event_type(stage), 'load', id, id, 'system', 'system',
            jsonb_build_object('load_id', load_id, 'source', load_board_source),
            NULL, stage, COALESCE(stage_updated_at, created_at), 'pipeline_loads', id, 'load-' || id
  ...
  },
  {
    table: 'pipeline_loads',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'load.researched', 'load', id, id, 'researcher', 'agent',
...
const AGENT_CALLS_STEPS: BackfillStep[] = [
  {
    table: 'agent_calls',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'call.initiated', 'call', id, pipeline_load_id, 'voice', 'agent',
  ...
  },
  {
    table: 'agent_calls',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'call.connected', 'call', id, pipeline_load_id, 'voice', 'agent',
  ...
  },
  {
    table: 'agent_calls',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'call.ended', 'call', id, pipeline_load_id, 'voice', 'agent',
  ...
  },
  {
    table: 'agent_calls',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'call.outcome_recorded', 'call', id, pipeline_load_id, 'voice', 'agent',
...
const AGENT_JOBS_STEPS: BackfillStep[] = [
  {
    table: 'agent_jobs',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'job.' || status, 'job', id, pipeline_load_id, queue_name, 'system',
...
const CONSENT_LOG_STEPS: BackfillStep[] = [
  {
    table: 'consent_log',
    sql: `SELECT fn_insert_event(
            fn_myra_tenant_id()::integer, 'consent.logged', 'consent', id, NULL, 'compliance-service', 'system',
...
const SCRAPER_RUNS_STEPS: BackfillStep[] = [
  {
    table: 'scraper_runs',
    sql: `SELECT fn_insert_event(
            COALESCE(tenant_id, fn_myra_tenant_id()::integer), 'scraper.run_completed', 'scraper_run', id, NULL, 'scanner', 'system',
