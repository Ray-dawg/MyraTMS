...
**Interfaces:**
- Consumes: `acquireLoadLock`/`releaseLoadLock`/`acquireCarrierPhoneLock`/`releaseCarrierPhoneLock` from `@/lib/pipeline/carrier-locks` (already exists, unchanged); `CARRIER_CALL_QUEUE_CONFIG.queueName` (Task 4, as the literal string `'carrier-call-queue'`). Does NOT consume `calculateCarrierNegotiationParams()` from Task 2 in this task's scope — the envelope check happens webhook-side (Task 3, on the call's outcome), not cascade-side (this task only decides which carrier to dial next, before any outcome exists to check). Live-mode dialing, which is explicitly out of scope and throws if reached, is where a future session would consume it.
