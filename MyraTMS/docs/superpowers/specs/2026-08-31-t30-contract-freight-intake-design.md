...
3. **`getMarginFloor(currency)` (`lib/tenants/margin-floor.ts`) is not what spec §4.3 assumes.** It's Myra-only (no `tenant_id` param), and returns a flat **dollar** floor ($270 CAD / $200 USD) — but §4.3 computes `impliedMargin` as a **percentage** and compares directly against it. Real unit mismatch in the spec. The correct per-tenant, percentage-based floor is `resolveMargin(tenantId, currency)` (`lib/pricing/resolve-margin.ts`), the same source T-21's `quotePricing()` already uses.
...
- **`lib/contract-intake/validate-rate.ts`** — `validateTenderedRate(tenantId, tender, marginFloorOverridePct?)`: calls `quotePricing()` once (direction `'sell'`, new `requestSource: 'contract_intake_validation'`), compares `tender.rate` against `cost.total` for `impliedMargin`, compares against `marginFloorOverridePct ?? negotiation.marginEnvelope.floor`.
...
## 5. New API surface (spec §6, minus the approve/reject endpoints folded into `PATCH /api/exceptions/[id]`)

```
POST /api/contract-intake/webhook              (email/attachment receipt — thin entry point calling the imap-poller's processing path)
