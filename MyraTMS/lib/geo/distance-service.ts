 ...
 * Results are cached in the distance_cache table (30-day TTL).
 */
...
export async function getDistance(originAddress: string, destAddress: string): Promise<DistanceResult> {
  ...
  try {
    ...
    // If geocoding fails, we can't proceed with the distance calculation
    throw new Error("Unable to geocode addresses. Ensure NEXT_PUBLIC_MAPBOX_TOKEN is configured or provide lat/lng coordinates.")
  }

  // Try Mapbox Directions for accurate driving distance
  ...
  if (directions) {
    ...
    driveTimeHours = distanceKm / 85 // ~85 km/h average truck speed
  }
  ...
  // Store in cache
  await sql`
    INSERT INTO distance_cache (origin_hash, dest_hash, origin_address, dest_address, distance_km, distance_miles, drive_time_hours, origin_lat, origin_lng, dest_lat, dest_lng)
    VALUES (${originHash}, ${destHash}, ${originAddress}, ${destAddress}, ${distanceKm}, ${distanceMiles}, ${driveTimeHours}, ${originCoords.lat}, ${originCoords.lng}, ${destCoords.lat}, ${destCoords.lng})
  `
...
/**
 * Alternative entry point when lat/lng are already known.
 * Skips geocoding, still uses directions API for driving distance.
 */
