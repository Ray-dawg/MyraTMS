...
// Body: { origin, destination, equipment?, maxAge? }
// Searches DAT + Truckstop APIs (if keys set), caches in Redis.
...
async function searchDAT(
  ...
  try {
    ...
    // Map DAT response to our schema
...
function deduplicateLoads(loads: LoadboardResult[]): LoadboardResult[] {
  ...
  for (const load of loads) {
    // Dedup key: similar origin+destination+pickup date
...
export async function POST(req: NextRequest) {
  ...
  const cacheKey = `loadboard:search:${(origin || "any").toLowerCase()}:${(destination || "any").toLowerCase()}:${(equipment || "any").toLowerCase()}`
  ...
  // If API keys exist, call real APIs
  if (apiConnected) {
  ...
  // Mock fallback -- filter mock data based on search params
