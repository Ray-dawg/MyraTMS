...
- 2026-08-25 — **E2-01 M1 Session 1 — All 5 tasks complete.** Historical backfill script (shadow mode) ships: `MyraTMS/scripts/e2_backfill_load_source.ts` + `__tests__/scripts/e2-backfill-load-source.test.ts`. Implements `backfillBatch(loadIds[], {force?})` — classifies pipeline_loads via `load-source-classifier`, skips already-classified rows (idempotent), routes unresolved to review verdict (never silent accept). **CRITICAL:** NEVER writes to `stage` or `qualification_reason` (shadow mode — Session 2+ adds Qualifier wiring + review routing). 4 tests pass: registry-hit classification, idempotency, force re-classification, review routing. Full test surface: 46/46 new E2-01 tests (verification/ + load-source-classifier.test.ts + seed-registry.test.ts + backfill.test.ts) + 101/103 existing pipeline tests (2 pre-existing ranker/events-backfill timeouts against live infra, not regressions). TypeScript clean. Commit `9b53327`. **Open items:** Sessions 2–4 (Qualifier wiring, review routing, M2 assertions) not yet built. Authorization/verification stack (authority-lookup, load-source-classifier, poster-registry) complete & production-ready for shadow backfill.
- 2026-08-25 — **E2-03 M0 "Stop the Bleeding" complete (sell-side).** New sibling PRD (`Engine 2/E2-03_Engine2_SellSide_Expansion_PRD.md`), built on the E2-02 investigation's finding that the Dispatcher auto-assigns a carrier to every booked load with **zero real carrier ever contacted**. Migration `041-sellside-expansion-schema.sql` (M0/M1/M4 schema, applied + verified live): `agent_calls.call_type` CHECK constraint (`'outbound_shipper'|'outbound_carrier'`, closes the shared-column collision risk E2-02 flagged), `pipeline_loads` carrier-outcome columns, `loads.carrier_cost_estimated`, `carriers` Gate-2 verification columns, plus `exceptions.pipeline_load_id`/`source_module`/`suggested_action`/`sla_due_at` added independently of E2-01's own (still-unmerged) `040` migration so neither PRD's migration depends on merge order. `dispatcher-worker.ts` changes: **(1)** new `CARRIER_AUTO_ASSIGN_ENABLED` flag (default `false` — the one flag across both PRD arcs that defaults OFF) gates the auto-assign branch; when off, the load escalates instead (`pipeline_loads.stage='escalated'` + a rich `exceptions` row with `carrier_id` set so the existing Alert Center's carrier-name join resolves, load context in `title`/`detail` text — reused the existing table/UI, built nothing new). **(2)** idempotency guard on `createTMSLoad()` — a `dispatch-queue` retry after a downstream failure no longer creates a second `loads` row for the same pipeline load (`tms_load_id` is the natural idempotency key). **(3)** `carrier_cost_estimated` honesty flag — when a carrier has no rate history, `fetchCarrierRate()`'s `0` fallback (previously indistinguishable from a genuine zero-cost 100%-margin load) is now flagged `estimated: true`, written to `loads.carrier_cost_estimated`, and surfaced on the load detail view (`components/load-quick-view.tsx`, small "(est.)" marker next to Carrier Pay). **(4)** finished a half-done cleanup: T-18/T-19 had already retired `AUTO_BOOK_PROFIT_THRESHOLD` at the config level but left 4 operational files (`run-workers.ts`, both sprint6-shadow preflight scripts, the runbook) still referencing it as if live — all 4 now say it's retired and point at `getMarginFloor()`. Built via subagent-driven-development (6 tasks, each independently reviewed clean) in an isolated worktree, separate from a concurrent session finishing E2-01 (buy-side) on its own branch. One real regression caught and fixed during final regression check: 2 pre-existing T-17 test files (`events-triggers.test.ts`, `events-views.test.ts`) inserted `agent_calls.call_type='negotiation'` — a placeholder value no live code path has ever written (confirmed: `retell-webhook.ts` only ever writes `'outbound_shipper'`) — the new CHECK constraint correctly caught it; fixed the 2 test files, not the constraint. Full pipeline + loadboards suite: 78/79 pass — the one failure is `ranker.test.ts`'s 30s timeout against the live 200-carrier pool, the same pre-existing environmental condition noted above, unrelated to this change. `tsc` clean. M2–M6 (the actual carrier-calling worker, negotiation envelope, rate-con send, Gate 2 wiring, lifecycle monitoring) not started — M0 was scoped standalone per the PRD's own sequencing rule.
- 2026-08-25 — **E2-03 M2 "Foundation" complete (sell-side) — shadow-only, no real call placed.** Session 2 of the M2 plan per the PRD's §13 build order, plus the leftover Session-1 item M0 didn't cover (§6.7's `call_type` webhook branch). Closes a second shared-column risk M0's own audit missed: `agent_calls.agreed_rate`/`profit`/`outcome` were shared between shipper and carrier calls with nothing but `call_type` to disambiguate a read — migration `042-carrier-call-columns.sql` gives carrier calls their own `carrier_agreed_rate`/`carrier_outcome`/`carrier_profit` columns, mirroring M0's fix on `pipeline_loads`. New `calculateCarrierNegotiationParams(agreedShipperRate, currency)` in `cost-calculator.ts` computes a payment ceiling (inverted from the existing shipper-side sale-floor function) using the same shared flat-dollar margin table — resolves a real inconsistency in PRD §6.5's own text (parameter named `minMarginPct` but its formula used a flat `minMarginFloor`) toward this codebase's established convention rather than a raw percentage. `retell-webhook.ts` gained a `call_type` routing branch: a new, fully parallel `processCarrierCallCompleted()` writes only to the new carrier-only columns, enforces the negotiation ceiling server-side (an above-ceiling "accept" is rewritten to `'escalated'`, never silently booked), and never touches `processCallCompleted()`/`parseCall()`/`writeAgentCall()` or the shipper path's shared columns — confirmed at final review: the whole 9-commit branch has 2 deletions total. New `carrier-call-queue` (10th BullMQ queue) and `carrier-voice-worker.ts` (cascade worker) compute and report a shadow-mode dial decision (which carrier, in what order) without ever making a real HTTP call — `CarrierVoiceWorker` isn't wired into `run-workers.ts`/`lib/workers/index.ts`, the queue has zero consumers repo-wide, and the live-dial branch is a deliberate `throw` (not a stub or no-op) gated behind a new `CARRIER_CALLS_ENABLED` flag that defaults false. Built via subagent-driven-development (5 tasks + a final whole-branch review + one fix wave, all independently reviewed). **Real bugs found and fixed during the process, not just at final review:** two plan-authoring bugs caught by implementers mid-task (a `parseCarrierTranscript()` regex that didn't match its own test fixture; a test file that seeded `match_results` before `carriers`, violating an FK, and never set `PIPELINE_ENABLED=true`, which would have shipped 5 tests silently passing on the kill-switch skip path without exercising any real cascade logic). The final whole-branch review (opus) then caught 5 further Important findings, most consequentially: carrier calls with a non-`'completed'` status (`failed`/`no_answer`/`voicemail`/`busy` — 3 of the PRD's 6 core cascade states) fell through the webhook's dispatch ladder into shipper-specific handlers that null out `agreed_rate`/`profit`/`stage`, unreachable today only because shadow mode never dials; also a missing `roundCurrency()` call on the new negotiation function (feeding a `DECIMAL(10,2)` column with unrounded values), a currency-source mismatch between the envelope computation and what gets persisted, a missing `transcript`/`recording_url` write on the carrier `agent_calls` INSERT, and the branch's two most load-bearing safety properties (the `CARRIER_CALLS_ENABLED` env-var *default* path, and the live-dial throw) having zero test coverage despite one test's name claiming otherwise. All fixed in one final fix wave, scoped-re-reviewed clean. Confirmed pre-existing, unrelated to this branch: 5 failing tests in `calculateTotalCost`/`estimateMargin`/`calculateNegotiationParams` (shipper-side, verified via direct test run + `git stash` bisection — worth its own future task); Engine 3's event-layer triggers (migration 035) only fire on UPDATE of `outcome`/`call_ended_at`, but every call (shipper or carrier) INSERTs those at creation time, so no `call.outcome_recorded`/`call.ended` event has ever fired for any call — pre-existing gap, flagged for whoever owns the T-17 metric views. `tsc` clean throughout. **Next session's precondition:** the non-`'completed'`-status routing fix (already shipped in this session) must stay in place before `CARRIER_CALLS_ENABLED` is ever flipped — live dialing itself (the cascade state machine, per-carrier-phone locking in use, real Claude-based carrier transcript parsing replacing the current regex placeholder) is still fully unbuilt.
