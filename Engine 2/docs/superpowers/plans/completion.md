...
- [ ] **A.3.1: Vercel — MyraTMS production deploy.** Connect Vercel project to GitHub repo, configure env vars (DATABASE_URL, KV_*, JWT_SECRET, RETELL_*, ANTHROPIC_API_KEY, PIPELINE_IMPORT_TOKEN, CRON_SECRET, PIPELINE_ENABLED=false initially, MAX_CONCURRENT_CALLS=0 initially, AUTO_BOOK_PROFIT_THRESHOLD=999999 initially). Configure custom domain.
- [ ] **A.3.2: Railway — worker host deploy.** Deploy `MyraTMS/scripts/run-workers.ts` as a long-running Railway service. Same env vars as Vercel (re-using same Neon + Upstash). 512 MB memory minimum. Region matched to Neon.
